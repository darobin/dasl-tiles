...
import { readFile, open } from 'node:fs/promise';
...
export default class TileWriter {
  ...
  async write (out) {
    ...
    const { fd: tmp, path: tmpPath, cleanup } = await tmpFile();
    ...
    for (const p of paths) {
      ...
      await tmp.writeFile(size);
      await tmp.writeFile(cid.bytes);
      await tmp.writeFile(buf);
    }
    await tmp.close();
    ...
    await outh.writeFile(size);
    await outh.writeFile(meta);
    const tmpRead = await open(tmpPath);
    await outh.writeFile(tmpRead.createReadStream());
    cleanup();
  }
}
